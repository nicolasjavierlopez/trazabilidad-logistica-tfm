...
import { useToken, useReceivedBalance, useRawMaterialWrite } from "@/hooks/useRawMaterial";
...
export default function TransferRequestFormView({ senderRole, recipientRole }: Props) {
  ...
  const isReceived = token ? token.creator.toLowerCase() !== address?.toLowerCase() : false;
  const { data: receivedBalanceData } = useReceivedBalance(isReceived ? tokenIdBig : undefined, address);
  const receivedBalance = receivedBalanceData as bigint | undefined;
            ...
            <Card variant="outlined">
              <CardContent sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <Box>
                  ...
                  <Typography variant="caption" className="font-bold">{maxBalance.toLocaleString()}</Typography>
                </Box>
              </CardContent>
            </Card>
