...
export default function TraceabilityDialog({ tokenId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const build = async () => {
      ...
      // Assign levels via recursive depth from roots
      ...
      // Group by level, sort within by createdAt
          ...
          <>
            ...
            <Box sx={{
              ...
              <svg width={svgW} height={svgH} style={{ display: "block" }}>
                <defs>
                  <marker id="fp-arr" viewBox="0 0 10 10" refX="8" refY="5"
            ...
            {selected && (
              <Box sx={{
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                  <Box sx={{ width: 12, height: 12, borderRadius: "50%", bgcolor: ROLE_COLORS[selected.role] ?? "#9e9e9e", flexShrink: 0 }} />
                  <Typography variant="subtitle2" sx={{ fontWeight: "bold" }}>{selected.name}</Typography>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0.75 }}>
                  {([
                    ...
                    ["Is leaf", selected.tokenId === tokenId?.toString() ? "Yes (this product)" : "No"],
